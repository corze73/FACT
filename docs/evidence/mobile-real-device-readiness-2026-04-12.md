...
- Physical iPhone detected by Xcode tooling: `C's Phone (18.6.2)` (currently offline)
- Blocking error:
  - `Signing for "FACTMobile" requires a development team. Select a development team in the Signing & Capabilities editor.`
...
3. No scripted `ios:release` npm command (manual `xcodebuild` path works, but a script would reduce operator error).
...
- Cross-platform (iOS + Android) pilot readiness: moderate (2-4 days with full regression pass)
